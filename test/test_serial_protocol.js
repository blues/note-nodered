...
class MockChannelReadWriter {
    ...
    ReadData = []
    WriteData = []
    WriteBufferSpaceMax = 250;
    WriteBufferSpaceAvailable = 250;
    LimitWriteBufferSize = false;

    constructor(config) {
        if(config === undefined)
            return;

        if( 'writeBufferLength' in config){
            this.WriteBufferSpaceAvailable = config.writeBufferLength;
            this.WriteBufferSpaceMax = config.writeBufferLength;
            this.LimitWriteBufferSize = true;
        }
